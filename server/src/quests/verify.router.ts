        ...
        if (questDocSnap.exists()) {
          ...
          if (isTwitter) {
            ...
            const response = await fetch(
              `https://api.ftmscan.com/api?module=account&action=txlist&address=${address}&startblock=57530880&endblock=99999999&page=1&offset=10&sort=desc&apikey=${process.env.API_KEY}`
            );
